...
output "database_url" {
  ...
  value       = "mysql://${aws_db_instance.fastfood_mysql.username}:${var.db_password}@${aws_db_instance.fastfood_mysql.endpoint}/${aws_db_instance.fastfood_mysql.db_name}?allowPublicKeyRetrieval=true"
  sensitive   = true
}
...
output "rds_security_group_id" {
  ...
  value       = data.aws_security_groups.eks_node_sg.ids[0]
}